...
;(function( $ ){ 
	...
	if( ! site_ajax_load.container || ! site_ajax_load.navigation )
		return false;
	...
	var wpAdminBar = '#wpadminbar',
		wrapper    = site_ajax_load.container, 
		navigation = site_ajax_load.navigation;
	...
	if( ! $body.hasClass( 'ajax-page-load' ) )
		return false;
	...
	//	jQuery( 'a[href^="' + wp_ajaxify.site_url + '"]:not(.no-wp-ajaxify, [href^="#"], [href*="wp-login"], [href*="wp-admin"], [href*="feed"])' ).on( 'click', function( event ){ 
		...
		jQuery( 'a:internal:not( .no-ajax, [href*="wp-login"], [href*="wp-admin"], [href*="feed"] )' ).on( 'click', function( event ){ 
			...
			History.pushState( null, site_ajax_load.loading_page_title, href );
	...
	$window.on( 'statechange', function( event ){ 
		...
		jQuery.ajax({
			...
			beforeSend: function( xhr ){ 
				...
				jQuery( 'html, body' ).stop( true, true ).animate({
					scrollTop: site_ajax_load.scroll_top_distance
		...
		//	setTimeout( function(){ 
			...
			{ 
				...
				if( site_ajax_load.admin_bar_showing )
